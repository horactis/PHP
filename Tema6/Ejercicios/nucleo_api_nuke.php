...
function controlDelete($_conexion, $entrada){
    if(!isset($entrada["nombre_desarrolladora"]) || $entrada["nombre_desarrolladora"] == ""){
        echo json_encode([
            "estado" => "error",
            "mensaje" => "no se ha encontrado una desarrolladora que eliminar"
        ]);
        return;
    }
    if($entrada["nombre_desarrolladora"] == "NUKE"){
        $consulta = $_conexion->prepare("DELETE * FROM videojuegos");
        $consulta = $_conexion->prepare("DELETE * FROM desarrolladoras");
    }else{
