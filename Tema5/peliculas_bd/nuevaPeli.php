...
<body>
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(!$errores){
             ...
             * 
             * ACID: Acrónimo de las cuatro claves fundamentales que definen una transaccion
             * 
             * Atomicidad: todo o nada (lo llevamos a cabo con commit() y rollback())
             * 
             * Consistencia: la BD no se quede en un estado inválido. Por ej: si tenemos que hacer dos operaciones, que no se haga solo una de ellas, o se hacen las dos o nunguna pero nunca a medias.
             * 
             * Aislamiento: una transaccion "no ve" resultados intermedios de otra transaccion
             * 
             * Durabilidad: tras el "commit" los cambios sobreviven a caídas del servicio
             * 
             * si realizamos varias opciones (insertar una peli y actualizar otra), no habra estados "a medias".
             */
            $consulta = "INSERT INTO peliculas (
                            titulo,
                            nombre_estudio
                            ...
                            duracion)VALUES(?, ?, ?, ?, ?)";
            try {
                $_conexion->begin_transaction(); //INICIAMOS LA TRANSACCIÓN
                ...
                $stmt->close();
                echo "<div class='alert alert-successs'> pelicula insertada correctamente {$e->getMessage()}</div>";
            } catch (mysqli_sql_exception $e) {
                $_conexion->rollback();
                echo "<div class='alert alert-warning'>No se ha podido insertar la peli correctamente {$e->getMessage()}</div>";
            }
        }
    }
    ?>
